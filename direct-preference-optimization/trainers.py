...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        rank0_print('Cache directory is:', get_local_dir(config.local_dirs))
        ...
        data_iterator_kwargs = dict(
            ...
            sft_mode=config.loss.name == 'sft',  # the default for eval is DPO mode, to evaluate preference accuracy using logps
        )
        ...
        if not config.eval_only:
            ...
            self.eval_iterator = get_batch_iterator(**data_iterator_kwargs, split='test', n_epochs=config.n_epochs, n_examples=config.n_eval_examples, batch_size=config.eval_batch_size, silent=rank != 0, cache_dir=get_local_dir(config.local_dirs))
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name == 'dpo': #or loss_config.name == 'none':
        ...
        elif loss_config.name == 'sft' or loss_config.name == 'none':
            policy_chosen_logits = self.policy(batch['chosen_input_ids'], attention_mask=batch['chosen_attention_mask']).logits.to(torch.float32)
            policy_chosen_logps = _get_batch_logps(policy_chosen_logits, batch['chosen_labels'], average_log_prob=False)

            losses = -policy_chosen_logps

            if loss_config.name == 'none':
                ...
                accuracies = (policy_chosen_logps > policy_rejected_logps).float()
                accuracies = all_gather_if_needed(accuracies, self.rank, self.world_size)
                metrics[f'accuracies_{train_test}'] = accuracies.cpu().numpy().tolist()
    ...
    def run_full_eval(self):
        """Generate samples from the policy, if doing DPO training) for the given batch of inputs."""
        if self.config.sample_during_eval:
            data_iterator_kwargs['sft_mode'] = True
            # one prompt only appears once in this dataset for inference purpose
            self.inference_eval_iterator = get_batch_iterator(**self.data_iterator_kwargs, split='test', n_epochs=1, n_examples=None, batch_size=self.config.eval_batch_size, silent=self.rank != 0, cache_dir=get_local_dir(self.config.local_dirs))

            self.inference_eval_batches = list(self.inference_eval_iterator)
            rank0_print(f'Loaded {len(self.inference_eval_batches)} inference eval batches of size {self.config.eval_batch_size}')

        self.eval_iterator = get_batch_iterator(**self.data_iterator_kwargs, split='test', n_epochs=1, n_examples=None, batch_size=self.config.eval_batch_size, silent=self.rank != 0, cache_dir=get_local_dir(self.config.local_dirs))
        self.eval_batches = list(self.eval_iterator)
        rank0_print(f'Loaded {len(self.eval_batches)} eval batches of size {self.config.eval_batch_size}')

        rank0_print(f'Running evaluation and inference mode!')
        self.policy.eval()
        ...
        if self.config.sample_during_eval:
            # sample from all eval batches
            for eval_batch in (tqdm.tqdm(self.inference_eval_batches, desc='Generating samples...') if self.rank == 0 else self.inference_eval_batches):
                local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank, self.world_size, self.rank)
